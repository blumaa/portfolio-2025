    ...
    () => {
      ...
      gsap.to(busRef.current, {
        ...
        onRepeat: () => {
          ...
          const currentScaleX = gsap.getProperty(
            busRef.current,
            "scaleX",
          ) as number;
          gsap.to(busRef.current, {
        ...
        onUpdate: function () {
          ...
          // Peak timing: 0.01 after reaching each peak
          // Peaks at x≈50, x≈1000, x≈2000 in path from -200 to 2200 (2400 total)
          // Peak1: 250/2400 ≈ 0.104 → trigger at 0.114
          // Peak2: 1200/2400 = 0.5 → trigger at 0.51
          // Peak3: 2200/2400 ≈ 0.917 → trigger at 0.927
          let afterPeak1, afterPeak2, afterPeak3;

          if (isReversed) {
            // Right to left: peaks encountered in reverse
            afterPeak3 = progress > 0.093 && progress < 0.103; // 1 - 0.917 + 0.01 ≈ 0.093
            afterPeak2 = progress > 0.51 && progress < 0.52; // 1 - 0.5 + 0.01 = 0.51
            afterPeak1 = progress > 0.906 && progress < 0.916; // 1 - 0.104 + 0.01 ≈ 0.906
          } else {
            // Left to right: normal order
            afterPeak1 = progress > 0.114 && progress < 0.124; // 0.104 + 0.01 = 0.114
            afterPeak2 = progress > 0.51 && progress < 0.52; // 0.5 + 0.01 = 0.51
            afterPeak3 = progress > 0.927 && progress < 0.937; // 0.917 + 0.01 = 0.927
          }
      ...
      [cloud1Ref, cloud2Ref, cloud3Ref, cloud4Ref].forEach((cloudRef, index) => {
        ...
        const randomYDistance = 15 + Math.random() * 35; // 15-30px
        const randomXDistance = 10 + Math.random() * 30; // 10-20px
    ...
    <Box display="flex" justifyContent="center" alignItems="center" width="100%" height="100%" >
      ...
      >
        ...
        <g fill={colorScheme === "dark" ? "#1E8449" : "#239B56"} opacity="0.6">
          {/* Far grass (smaller) */}
          {[80, 170, 240, 330, 510, 620, 730, 890, 980, 1090, 1210, 1380, 1520, 1670, 1790, 1880].map((x, i) => {
            ...
            return (
              <g key={`far-${i}`} transform={`translate(${x}, ${1500 + yOffset})`}>
                <path d={`M 0 0 Q -${curve} -${height1 * 0.5} 0 -${height1}`} stroke={colorScheme === "dark" ? "#1E8449" : "#239B56"} strokeWidth="1.5" fill="none" />
                <path d={`M 0 0 Q ${curve} -${height2 * 0.5} 0 -${height2}`} stroke={colorScheme === "dark" ? "#1E8449" : "#239B56"} strokeWidth="1.5" fill="none" />
              </g>
            );
          })}
          ...
          {[120, 270, 390, 540, 690, 820, 970, 1110, 1260, 1420, 1580, 1720, 1860].map((x, i) => {
            ...
            return (
              <g key={`mid-${i}`} transform={`translate(${x}, ${1550 + yOffset})`}>
                <path d={`M 0 0 Q -${curve} -${height1 * 0.5} 0 -${height1}`} stroke={colorScheme === "dark" ? "#1E8449" : "#239B56"} strokeWidth="2" fill="none" />
                <path d={`M 0 0 Q ${curve} -${height2 * 0.5} 0 -${height2}`} stroke={colorScheme === "dark" ? "#1E8449" : "#239B56"} strokeWidth="2" fill="none" />
              </g>
            );
          })}

          {/* Near grass (larger) - in front */}
          {[40, 160, 310, 460, 580, 750, 890, 1050, 1190, 1350, 1490, 1640, 1790, 1940].map((x, i) => {
            ...
            return (
              <g key={`near-${i}`} transform={`translate(${x}, ${1620 + yOffset})`}>
                <path d={`M 0 0 Q -${curve} -${height1 * 0.5} 0 -${height1}`} stroke={colorScheme === "dark" ? "#1E8449" : "#239B56"} strokeWidth="2.5" fill="none" />
                <path d={`M 0 0 Q ${curve} -${height2 * 0.5} 0 -${height2}`} stroke={colorScheme === "dark" ? "#1E8449" : "#239B56"} strokeWidth="2.5" fill="none" />
              </g>
            );
          })}
        </g>
