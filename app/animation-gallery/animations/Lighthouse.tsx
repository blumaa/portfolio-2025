    ...
    () => {
      ...
      gsap.to(state, {
        ...
        onUpdate: () => {
          ...
          // Beam visibility: sine wave, clipped to positive
          const rightVis = Math.max(0, sin);
          const leftVis = Math.max(0, -sin);

          // Depth: normalized cosine [0, 1]
          // 1 = facing viewer (close), 0 = facing away (far)
          const depth = (1 + cos) / 2;

          // Beam spread based on depth
          const farHalf = FAR_HALF_MIN + depth * (FAR_HALF_MAX - FAR_HALF_MIN);

          // Glow/flash: only when facing viewer
